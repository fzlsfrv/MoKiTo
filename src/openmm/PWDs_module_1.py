...
def generate_PWDistances_torch(
    ...
    Dt = pt.zeros((Npoints, Nfinpoints, Ndims), dtype = pt.float32, device=device)
    print(Dt.shape)
    ...
    for f in range(len(fs_folders)):
        ...
        for i in tqdm(range(int(Npoints//6))):
            ...
            for j in range(Nfinpoints):
                ...
                Dt[int(d*f + i),j,:]  =  pt.tensor(dt, dtype=pt.float32, device=device)
                                
    
    pt.save(Dt, iso_dir + 'PWDistances_t_1_full_frame_9.pt')
