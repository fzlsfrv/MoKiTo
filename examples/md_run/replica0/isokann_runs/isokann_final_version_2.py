...
# Load from the file
with open(out_isokann + 'hyperparameters_final.pkl', 'rb') as file:
...
# Load initial and final states and convert to torch
    ...
    if i > 0 and os.path.exists(out_isokann +  f"nn_chk_1/f_NN_chk_{i-1}.pt"):
        f_NN.load_state_dict(pt.load(out_isokann +  f"nn_chk_1/f_NN_chk_{i-1}.pt", map_location=device))
    ...
    pt.save(f_NN.state_dict(),out_isokann +  f"nn_chk_1/f_NN_chk_{i}.pt")
...
pt.save(f_NN.state_dict(), out_isokann  + 'nn_chk_1/f_NN.pt')
...
chi = np.load(out_isokann + 'chi0_1.npy', allow_pickle=True)
train_LOSS = np.load(out_isokann + 'train_LOSS_1.npy', allow_pickle=True)
val_LOSS = np.load(out_isokann + 'val_LOSS_1.npy', allow_pickle=True)
