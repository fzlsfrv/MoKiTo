...
def trainNN(net,
    ...
    #scheduler = pt.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=max(1, patience // 2))
...
def power_method(   pt_x0,
    ...
    for i in loop:
        ...
        with pt.no_grad():
            for batch_xt in xt_loader:
                batch_xt = batch_xt.to(device)
                chi_batches.append(f_NN(batch_xt))
