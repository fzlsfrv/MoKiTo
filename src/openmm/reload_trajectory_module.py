...
from src.useful_functions import get_latest_state_file
...
def continue_trajectory(
    ...
    print("Loading the positions...")
    print("The latest state file: ", get_latest_state_file(out_dir + 'initial_states/'))
    simulation.loadState(get_latest_state_file(out_dir + 'initial_states/'))
    print(f'Current simulation step: {simulation.currentStep}')
    ...
    if save_vels == 1:
        ...
        s = int(simulation.currentStep)
        i = (s // Nout_water)
        print(f'Generating the {i}th state file...')
        for step in range(s, Nsteps + 1):
            simulation.step(1)
