...
def generate_PWDistances_torch(
    ...
    if BB == True:
        ...
        d0 = np.zeros((Ndims,int(Npoints//2)))
        for i in tqdm(range(0, int(Npoints), 2)):
            traj.trajectory[i]
            atoms   =  traj.atoms[sel_idx]
            atom_coords = atoms.positions
            box_ = traj.trajectory.ts.dimensions
            d0[:, int(i//2)] = self_distance_array(atom_coords, box = box_)
    ...
    pt.save(D0, iso_dir + 'PWDistances_0_half_1.pt')
    ...
    d = int(Npoints//(6*2)) 
    for f in range(len(fs_folders)):
        print(f"fs_folders[{f}] =", fs_folders[f])
        for i in tqdm(range(0,int(Npoints//6),2)):
            ...
            for j in range(Nfinpoints):
                ...
                for k in range(Ntimesteps):
                    ...
                    Dt[k,int(d*f + (i//2)),j,:]  =  pt.tensor(dt, dtype=pt.float32, device=device)
                                
    
    pt.save(Dt, iso_dir + 'PWDistances_t_half_1.pt')
